...
export class MaquilleuseComponent implements OnInit {
  ...
  set maquilleuseSearch(value: string) {
    ...
    this.filtredmaq = this.maquilleuseSearch
      ...
      : this.maquillieuses$;
      this.pageSlice = this.filtredmaq.slice(0, this.pageSize);
  ...
  openModal(id) {
   ...
   search(searchBy){
    ...
    onPageChange(event: PageEvent) {
        ...
        if (endIndex > this.filtredmaq.length) {
            endIndex = this.filtredmaq.length;
        }
        this.pageSlice = this.filtredmaq.slice(startIndex, endIndex);
